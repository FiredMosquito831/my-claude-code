...
| **Desktop apps** | The applications MCC does not launch — Codex desktop, Goose, OpenCode, VS Code's Copilot custom endpoint, Crush, Roo Code and Antigravity's `agy` — get one **Configure** button on the Coding agents page that writes MCC's keys into the one file each reads and carries every other byte through unchanged, with a `.mcc-backup` taken once before the first edit, a diff preview before anything is written, a drift badge when an owned key is hand-edited, and **two undo modes**: remove MCC's keys, or restore the values MCC replaced. Claude Desktop has a button too since 6.56.0: Anthropic's MDM documentation names the local configuration library (`%LOCALAPPDATA%\Claude-3p\configLibrary\`, and `~/Library/Application Support/Claude-3p/configLibrary/` on macOS), so MCC owns one document there and merges a single foreign key -- and because that library is the *lowest*-precedence source the app reads, MCC probes `HKLM`/`HKCU\SOFTWARE\Policies\Claude` (macOS: Managed Preferences) first and refuses to write where a managed profile would silently win. Apps that cannot be pointed here at all get a dated reason instead of silence. Headless: `mcc-apps list | status | configure [--preview] | undo [--restore]`. |
...
| **Catalogues that re-read themselves, and facts MCC learned** | Every usable provider's model list is refetched hourly in the background, with the Models page saying when it last happened and when it happens next. Alongside it, every negative this deployment taught MCC — an output ceiling named in a 400, a reasoning field it refuses, a model that turned out to be blind — is stored, shown with its evidence, expired on a clock sized to how strong that evidence was, and forgettable per row, per provider or entirely. See [Catalogue Refresh And Learned Facts](#catalogue-refresh-and-learned-facts). |
...
| **Images, sized before they are sent** | Outbound pictures are resized to a 1568&nbsp;px long edge on the copy the router already takes — token-neutral on three of the four billing families, a 41% saving on the fourth, and never a mutation of your client's own request. A tier whose model cannot see can have each image *described* by a vision model instead of handing the whole request over, with descriptions cached on the image itself. See [Outbound Image Size](#outbound-image-size). |
...
| **Cost, with provenance** | Every request is priced once as it is logged, from the first rung of a ladder that answers — the host's own figure, models.dev, LiteLLM, a cross-provider vote — and the rung is stored beside the amount. Reported and estimated totals are shown side by side and never summed; a request nothing could price shows a dash rather than `$0.00`, and every total carries an “N of M priced” denominator. See [Cost Estimation](#cost-estimation). |
...
Pictures are resized before they are sent, once per attempt, on the deep copy the router already takes — so every dialect converter is handed pre-shrunk data and your client's own request is never mutated.

| Setting | Default | What it does |
| --- | --- | --- |
| `IMAGE_MAX_LONG_EDGE` | `1568` | Longest edge, in pixels, any outbound image is resized down to. Ships **on**, at the budget Anthropic itself resizes to: token-neutral on three of the four billing families because they already do this server-side, and a real 41% saving on the fourth (OpenAI's patch-billed models), which is also the one case where the model genuinely sees less. `0` sends exactly what the client sent. |
| `IMAGE_JPEG_QUALITY` | `0` | Re-encode a resized image as JPEG at this quality. `0` — the default — never changes the format, because these are screenshots of text and code and that is the worst case there is for JPEG ringing. Any image with an alpha channel is skipped whatever this says. |
| `IMAGE_DETAIL` | `auto` | OpenAI's per-image fidelity field. `auto` emits no field at all, which is unchanged behaviour; `low` and `high` are meaningless outside the OpenAI dialects and are not emitted there. |
| `TOOL_RESULT_IMAGE_DELIVERY` | `auto` | How an image a *tool* returned reaches the model. `auto` attaches it unless the answering model is published as unable to read one, in which case a plain sentence naming the tool takes its place; `attach` always sends it; `strip` never does. There is deliberately no option to go back to base64 text. |
| `VISION_ADAPTER_MODE` | `route` | What the [vision adapter](#vision-adapter) does. `route` hands the whole request to `MODEL_VISION`; `describe` sends each image to the vision chain on its own and lets the route's own model answer with the description in hand. |

**Was the estimate right?** A provider row on the **Models** page carries a `billed/est` chip once there is enough traffic to compare — input tokens the host actually billed against what MCC's per-family estimator predicted. A ratio near `1.0×` means the estimator has that family right; anything else is the only thing that can audit it. The request detail shows the same comparison per request, and adds a `+ adapter` line when a describe hop contributed tokens of its own.

### Cost Estimation

Every request is priced once, at the moment it is written to the log, and the amount is stored with the name of the source that produced it. It is never recomputed on read: a price that changes next month must not silently rewrite last month's bill.

The ladder is the same one every other model fact walks — the host's own reported cost, then models.dev's bucket for this provider, then LiteLLM's price map, then a vote across same-named rows in *other* providers' catalogues, then nothing. **One request is priced from one rung, whole**: a source that names an input rate but no cache-read rate has not priced a request that read from cache, and that request falls to the next rung entire rather than being stitched together into a number nobody published.

| Setting | Default | What it does |
| --- | --- | --- |
| `COST_ESTIMATION_ENABLED` | `true` | Master switch. Off, nothing new is priced; requests already priced keep their figures. |
| `COST_ESTIMATION_MODE` | `auto` | `auto` walks the whole ladder. `reported_only` stores a host's own figure or nothing, which is the strictest reading of "never guess". `computed_only` skips the host and always computes — how you audit a provider's own billing against a published price. |
| `COST_SOURCE_LITELLM_ENABLED` | `false` | The second price source. Off because turning it on adds a 2.3 MB cached artefact and one more conditional GET a day; it buys the misses, and the only published reasoning rate. Its refresh is integrity-checked — a minimum entry count and no shrink past half of what is cached — because it is served from a branch that has shipped broken JSON before. |

**A request nothing could price shows a dash, never `$0.00`.** Those are different facts, and every cost total on **Analytics** therefore carries an "N of M priced" denominator so a partial total cannot be read as a complete one. Reported and estimated amounts are shown side by side and are **never added together**. Nothing is backfilled: requests logged before 6.54.0 stay unpriced forever, because pricing them at today's rates would produce a confident number that was never anybody's bill.

The **Models** page shows the resolved input, output, cache and reasoning rates per model in USD per million tokens, each tagged with the rung it came from.

### Catalogue Refresh And Learned Facts

| Setting | Default | What it does |
| --- | --- | --- |
| `MODEL_DISCOVERY_REFRESH_SECONDS` | `3600` | Seconds between background refetches of every usable provider's `/models`. The **Models** page says when the last one happened and when the next is due; the server log says `catalogue changed: +N -M` when the list actually moved. `0` turns the sweep off, and the same line says so rather than going quiet. Unrelated to `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY`, which is a Claude Code variable MCC writes and never reads. |
| `MODEL_PROBE_NEW_MODELS` | `false` | Whether the sweep probes models it has just discovered, rather than waiting for a real request or a manual **Probe capabilities**. |

Every negative MCC holds about a deployment was learned from something that deployment itself said — an output ceiling named in a 400, a retry that only succeeded once a reasoning field was dropped, a probe that came back without vision. They are persisted to `~/.mcc/learned_facts.json` so a restart does not re-pay every rejection, and they **expire**, because MCC stops sending the thing that would produce a positive and a host that raises its own cap has no way to tell us. Three evidence classes, three clocks: a number or enum the host **stated** lasts 30 days, a refusal **inferred** from "it worked once the field was gone" lasts 7 days, and a **withheld model id** — the weakest negative here — lasts 72 hours.

**Expiry never deletes.** A fact past its clock is still loaded and still shown, marked *stale*, and simply stops being applied; the next real request re-pays one rejection and the row is fresh again. The **Models** page has a **Learned** filter chip, a per-row **Forget**, a per-provider forget on the provider card, and a forget-everything action under **Requests**. No response text is ever stored — the evidence is a bounded, redacted excerpt, and for a probe it is a status word and at most an HTTP code.
