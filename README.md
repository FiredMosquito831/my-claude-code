...
Add `--restart` (PowerShell: `-Restart`) to any of these and the install command also restarts the server for that configuration directory — it stops the one server bound to its `PORT` by that server's exact process ids, starts `mcc-server` again, and waits until `/health` answers. Every other My Claude Code server on the machine is listed and left running, and a port held by something that is not ours is reported rather than touched. `--no-start` (or `MCC_INSTALL_NO_START=1`) starts nothing at all. See [Updating](docs/USAGE.md#the-installer-restarts-the-server).
