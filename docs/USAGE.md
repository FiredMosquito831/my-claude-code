...
| WebView2 | Detected through the `EdgeUpdate` client key Microsoft documents, **and** by checking that the version it names has its files on disk — a registry entry whose runtime directory is gone counts as absent. Windows 11 ships the runtime; Windows 10 has had it pushed since December 2022. Only when it is genuinely missing is Microsoft's 1.8 MB Evergreen Bootstrapper run silently — and since 7.35.2 that bootstrapper **travels inside the installer** rather than being downloaded mid-install, so an offline machine still gets it. If it fails, setup says so and continues rather than refusing to install. |
| Visual C++ runtime | Not needed. The executable is built with a **statically linked CRT**, so it imports nothing from the Visual C++ redistributable. Before 7.35.2 it imported `VCRUNTIME140.dll`, and on a machine that had never installed the redistributable Windows killed the process at load time with `0xC0000135` — no window, no message. The release smoke now reads the binary's import table and fails if any redistributable DLL comes back. |
...
A provider holds a **pool** of keys, not a single value. Each key in the pool shows its own health (healthy, cooling down, locked out) and has its own **Remove**, which also takes effect immediately.

**The order of the pool is the order failover reads**, and since 7.29.0 you can change it here instead of retyping the variable: drag a key by its grip, or press **Move up** / **Move down** (ArrowUp / ArrowDown work on a focused grip). It applies at once with no restart, and the line above the pool says what moved and offers **Undo**. It also rebuilds the pool, so that pool's health counters start again -- the same thing adding or removing a key has always done, now said out loud.

**You can name a key.** Type a name in the box beside it, or in the optional **Name** field when you add one. A named key then reads as its name everywhere a credential is shown -- the card, the request log row and its detail modal, the retry ladder, the per-key analytics and the CSV -- with its masked label still on hover. Names live in `~/.mcc/credential_names.json`, stored against `sha256(key)[:16]` rather than the key, and they never leave this machine: not in a request, not in the request log, not in a rollup. Because the name is only a display join, renaming a key rewrites nothing that was already recorded, and a masked label that two keys in the pool share shows no name at all rather than the wrong one. If you added more than one key, pick a **Rotation** policy and press **Apply** — rotation is a restart-required setting, so the server restarts when you apply it.
...
**Several accounts, since 7.30.0.** Once one account is stored, "Sign in with Anthropic" becomes **Sign in another account**, and the card shows one row per account — name, plan, both expiries, scopes and that account's own usage windows — each with its own **Refresh now** and **Disconnect**. Signing the same account in again updates it in place and keeps the name you gave it; nothing is ever replaced or duplicated. Each account is a slot in the ordinary credential pool, so `ANTHROPIC_OAUTH_ACCESS_TOKEN_ROTATION` and the health ladder apply per account, and the request log and every export show the account's name where a key's name would be. Names default to the account's email address, which the token response already carries — MCC still never fetches the profile — and you can change or clear them. `mcc-anthropic-oauth-login --list` and `--remove <id>` do the same from a terminal, and neither ever prints a token.

**A refreshed token can go back where it came from.** For an account you **imported** from Claude Code, MCC writes the refreshed token back into `~/.claude/.credentials.json` so your real Claude Code session keeps working rather than finding its refresh token rotated away. It takes Claude Code's own `~/.claude/.storage-write` lock first and skips rather than forces if it cannot get it; it re-reads the file inside that lock and skips again if that file already holds a token at least as new; and it replaces only `claudeAiOauth`, leaving `mcpOAuth` and everything else untouched, after one `.bak-<epoch>`. It never applies to an account MCC signed in itself, and it is a no-op on macOS, where Claude Code keeps the credential in the keychain. `ANTHROPIC_OAUTH_WRITE_BACK=false` turns it off; `CHATGPT_OAUTH_WRITE_BACK` is the Codex equivalent.

**Downgrade note.** A 7.29.x build reads the **first** account out of the new store and serves it correctly, but the first write it performs replaces the whole document and drops the others. The `.bak-<epoch>` taken when the store was migrated is the recovery path.
...
Keys for a custom provider live in **`~/.mcc/custom_providers.json`, not `~/.mcc/.env`.** There is no environment variable for them, so the `{ENV}_API_KEY` / `{ENV}_ROTATION` file workflow does not apply — but the pool itself is the same one built-in providers use, so several keys plus a rotation policy work exactly as they do elsewhere. That includes the rail: drag or **Move up** / **Move down** to change which key failover reaches first, and name a key to have it read as that name everywhere. Names for a custom pool live in the same `~/.mcc/credential_names.json` a built-in pool's do.
...
> **A gap in the stored rows, 6.62.0 to 7.56.0: clients that hang up on the last event.** From 6.62.0 the row's arithmetic (thumbnails, token estimate, price) runs off the event loop, and a client that closed its connection the instant it read the terminal event (`response.completed`, `message_stop`, `[DONE]`) cancelled the request while that arithmetic was still running — and the row was dropped. Codex CLI always closes that way, so **no Codex success was recorded from 6.62.0 until 7.56.1**; any other early-closing client lost rows the same way, on all four surfaces. Their tokens, cost, key and harness attribution are missing from every count on this page for that span, and nothing can backfill them. From 7.56.1 the row is committed as soon as the arithmetic finishes, whether or not the client is still connected.
...
| **429** | the **(key, model) pair** is benched for **exactly the provider's `Retry-After`** — parsed from `Retry-After`, `retry-after-ms` or `x-ratelimit-reset-*` — or `RATE_LIMIT_COOLDOWN_SECONDS` (60 s) when the host publishes no header, capped at `RATE_LIMIT_COOLDOWN_MAX_SECONDS` (default 3600) for a header either way, and `RATE_LIMIT_COOLDOWN_MODE` decides whether any bench happens at all (`provider` / `fixed` / `off`). The key itself is benched only once `CREDENTIAL_MODEL_BENCH_ESCALATION` different models hold a live bench on it | rotates to the next key |
...
> **Successes on every surface count (7.56.2).** A model's successes are part of this arithmetic: in `rate_based` mode each one adds a success to the window, and in `legacy` mode one success resets the streak and lifts an active bench. Up to 7.56.1 only `/v1/messages` successes counted. The `/v1/responses`, `/v1/chat/completions` and Gemini adapters stopped reading at the answer's final event, so MCC never finished that attempt. Their successes never reached the bench, and their requests were stored with no attempt row. From 7.56.2 those adapters read the stream to its end, and a model used through Codex, Gemini CLI, Qwen Code or an OpenAI SDK has its successes counted the same way. Two things follow from this. Such a model is benched less often in `rate_based` mode, and a client that reads to the end waits for the upstream's own end of stream. Whatever arrives after the final event is read and thrown away, including a failure, so it never becomes an error frame for the client and never turns the request row into an error.
...
#### One update path (6.82.0)

Pressing **Update** in the dashboard now runs the same command you would type by
hand. There is one downloader, one thing that checks the wheel's checksum, one
installer and one owner of the restart, and all four are the install command.

Before this there were two of each. The dashboard's update lived in a
thousand-line PowerShell script generated by the server; the hand-run install
lived in `install.ps1`; they downloaded the wheel separately, verified it
separately, and disagreed about who restarts the server. On 2026-09-11 both ran
within three minutes of each other, both exited 0, and the machine had no server
for fifteen minutes because each of them believed somebody else owned the
restart.

**What the install command does now, whichever door you came in by:**

| Stage | What happens | Is the server down? |
| --- | --- | --- |
| `staging` | The new version is built **beside** the running one, in a directory of its own. | No. It keeps serving. |
| `verifying` | The staged version is **run** once — `mcc-server --version`, and an import of the package. A release that installs but cannot import itself is caught here. | No. |
| `stopping` | Exactly the one server on the configured port is stopped by its exact process ids. | From here. |
| `swapping` | Two directory renames. Measured at 2.7–20 ms. | Milliseconds. |
| `starting` | `mcc-server` is started detached and `/health` is polled. | Until it answers. |
| `done` | A listener answered. Only now is the previous copy swept. | No. |
| `rolling-back` → `recovered` | It never answered, so the previous version goes back and **it** is started. | Until the old one answers. |

The outage is the stop, the rename and the start — not the download, the
dependency resolve and the environment build, which used to happen with the old
version already deleted. The launcher files in `uv`'s bin directory are never
written at all: each one is a stub that runs whatever environment is at the
canonical path, so the rename alone cuts the whole machine over and no locked
`.exe` can abort anything.

**If the new version never answers**, the previous one is put back and started,
and the receipt ends at `recovered` rather than `failed`. The copy that made the
rollback possible is deleted only *after* the new server answers.

> **The update that installs 6.82.0 still runs today's helper.** The new path
> applies from the update after it. That is unavoidable: an update is carried
> out by the version you are leaving, not the one you are arriving at.

`install.cmd` also stops hard-coding `main` when it fetches `install.ps1`.
`--version 6.63.0` now fetches the installer that shipped with 6.63.0, and
setting `MCC_INSTALL_REF` to a branch, tag or commit fetches that one — which is
how a change to the installer is proven before it is merged.
...
If the new server never answers `/health`, the receipt records what was done
about it: `recovered` when the previous version was put back and started (the
ordinary case since 6.82.0, see below), and `failed` with the child's exit code,
...
| `STREAM_KEEPALIVE_IDLE_SECONDS` | `30` | **New in 7.46.0.** Seconds a streaming response may be silent -- before the first frame, or after a model that started goes quiet -- before MCC writes a keepalive the client's protocol defines as meaningless: `event: ping` on `/v1/messages`, an SSE comment (`: keepalive`) on `/v1/chat/completions`, `/v1/responses` and Gemini. It helps OpenAI and Gemini SDK clients (their timeouts count bytes) and any proxy between the client and MCC that closes idle connections. **It does not help Claude Code by default:** the official Anthropic SDK drops `ping` before its idle timer sees it; only with `_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL=1` in Claude Code's environment do the bytes count. A keepalive sent before any model answered commits the HTTP 200, so a failure after it arrives as an error inside the stream rather than an HTTP status. Every other byte is identical with it on or off. `0` never sends one. On Limits & Resilience → Stream keepalive. |
| `STREAM_KEEPALIVE_INTERVAL_SECONDS` | `20` | **New in 7.46.0.** Seconds between keepalives while the silence lasts. A real frame from the model resets the clock. |
| `STREAM_KEEPALIVE_MAX_SECONDS` | `300` | **New in 7.46.0.** Longest single stretch of silence MCC keeps alive; past it no more keepalives are written and the client's own idle timer runs exactly as with keepalives off, so a model that never answers cannot hold a client longer than this plus that timer. `0` = no cap. |
| `STREAM_KEEPALIVE_MODE` | `ping` | **New in 7.47.0, opt-in.** What the keepalive is on `/v1/messages`. `ping` (default) is Anthropic's `event: ping`, which the official Anthropic SDK -- and so Claude Code -- drops. `frames`: while the model's own text or tool-call block is open and silent, the keepalive is an empty delta of that same block (`text_delta` `""` / `input_json_delta` `""`), which Claude Code's idle timer does count and which adds nothing to the answer. It helps a model that has started answering and then goes quiet; before the first block, between blocks and while the model is thinking it is still a ping, and it never invents a thinking block. The wire shape changes, the content does not. Each request's row records how many frames it sent (**Keepalive frames** in the request detail). Other surfaces are unaffected. |
