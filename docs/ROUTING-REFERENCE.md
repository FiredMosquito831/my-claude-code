...
**A third, and it is not a number: `response_surface`.** It names which wire API one model is posted to -- `chat_completions`, `responses` or `messages` -- and it never reaches a body. Precedence for it is its own, and unchanged since 6.74.0: the override, then what a probe of this deployment proved, then what the vendor's published registry says (models.dev's per-model `provider.npm`), then Chat Completions. Whatever wins is folded against what the **provider declares it serves**, and a surface outside that list resolves to `unservable` with the reason shown on the Models page rather than the model disappearing.
