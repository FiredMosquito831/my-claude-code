...
**An entry that cannot serve says so.** When a rail entry's provider has no usable credential right now, the row shows a warning under the ref with a link to that provider's card. There are three warnings. *No usable credentials — sign in* means a ChatGPT or Claude subscription has nothing signed in, which includes a setting that still points at a retired store. *No usable credentials — add a key* means the provider has no key. *Every key benched until HH:MM* means every key in the pool is benched. Routing is unchanged: the router still tries the entry, fails it at once and moves to the next one, exactly as the request log records. The server works out the warning from what it already holds: the settings, the sign-in stores on disk, and the key health of providers it has already built. It never calls upstream to do so. The warning refreshes whenever the page reloads its status, for example after Apply or after you add a key. On Analytics, every widget that aggregates history shows the window it covers. **Failover**, **Vision adapter** and **Provider performance** also show when the settings were last saved, so a chain from before a change is not read as the current config.
...
| `STREAM_KEEPALIVE_MODE` | `ping` | **7.47.0, opt-in.** What the keepalive is on `/v1/messages`. `ping` is Anthropic's `event: ping`, which Claude Code drops. `frames`: while the model's own text or tool-call block is open (after its `content_block_start`, before its `content_block_stop`), the keepalive is an **empty delta of that block** -- `text_delta` with `""` or `input_json_delta` with `""` -- which Claude Code's idle timer does count. The text and tool arguments the client assembles are identical with it on or off; the bytes on the wire are not, which is why it is off by default. Anywhere else -- before `message_start`, before the first block, between blocks, inside a thinking block -- it is still a ping, and it never invents a thinking block. The request log records how many frames each request carried (`keepalive_frames`). Other surfaces keep their SSE comment. |
...
Since 7.21.0 a fetch finishes the job. It reads the lists, merges and ranks them exactly as before, and then **tests every address it found** with the same check the Add button runs — a TCP connect, a `CONNECT` tunnel, and a strict-TLS request to the chosen provider's own host. Only the addresses that passed are stored and offered. An address that did not answer is not kept. An address whose tunnel broke certificate validation is recorded **refused**, durably, so no later fetch offers it again — and, as always, that verdict is lifted only by later tests that *succeed*. Since 7.52.4 it takes **two passes in a row**: over five runs on 1,000 feed addresses, 2 of the 69 ever refused were refused in some runs and passed in another, so one lucky pass could let an intercepting address back into a chain. A pass that does not yet lift the refusal is filed as refused, with a note on the row saying how far the lift has got ("passed 1 of the 2 checks in a row that lift this refusal"). A check that did not answer breaks the row, and a restart starts the count again. Your credential was never at risk either way: nothing is sent over a connection that failed verification.
...
**A list's `https` label means an HTTP proxy (7.52.3).** Lists publish `https` for an HTTP proxy that can tunnel HTTPS -- one that answers `CONNECT` -- not for a proxy you must reach over TLS yourself. Up to 7.52.2 MCC took the label literally and dialled those addresses as `https://` proxies, and none of them could ever pass: measured on 160 such addresses, 0 passed as `https://` and 23 passed as `http://`. Since 7.52.3 a labelled `https` address is dialled as `http://`, and the row still says what the list claimed about HTTPS. A plain-text list line that writes `https://` in front of its own address keeps it, because that was written rather than labelled. Nothing already in a chain is rewritten: an `https://` address you added earlier stays as it is, and the next fetch offers the same address again as `http://` if it works that way.
...
**Screen, then confirm (7.53.0).** Most addresses a single test calls dead are not dead, they are unreliable: on 1,000 feed addresses, re-tested patiently three times, 31 % of the ones a fetch had thrown away worked at least once, and an address that works at all passes about 46 % of its tries. So the sweep above is now only the **screen**. Everything it called dead — except a proxy that refused the connection outright, which recovered only 13 % of the time — is re-tested in up to `PROXY_CHECK_CONFIRM_ATTEMPTS` confirm rounds (default 3), `PROXY_CHECK_CONFIRM_SPACING_SECONDS` apart (default 30 s; the first round starts one spacing after the screen), at the fetch's own concurrency, with the **live** connect limit `PROXY_CONNECT_TIMEOUT_SECONDS` instead of the screen's 5 s — an address the confirm stage admits is one live traffic would also wait for. A pass in any round is stored like a screen pass; only an address that failed every round is **confirmed dead**. Measured: +162 working (166 → 328), about 3.5 minutes added. Stop ends the confirm stage at once, including a spacing wait. Every pass is labelled **working**, **slow** (setup above `PROXY_CHECK_SLOW_MS`) or **flaky** (passed on fewer than half of three or more tries in this fetch); all three are kept and addable, and the candidate row shows the label as a chip. With `PROXY_CHECK_LINK_GUARD` on, the fetch also pauses — marking nothing dead — while your own connection to the provider is failing.

The same rule reaches the other checkers. The **health re-prober** gives an address whose re-check is due a round of `PROXY_CHECK_CONFIRM_ATTEMPTS` tries: any pass puts it back; only when all fail does it move one rung down the 1 minute / 5 minutes / 1 hour ladder, and the row says so — *unhealthy — failed 3 of 3 tries at 14:02; next round in 59m*. Before, one unlucky try moved it a whole rung. **Test all** and **Add all working** use the same round; the single-row **Test** stays one try. And an address that fails on live traffic for the first time is re-tested once on the re-prober's next tick (within 30 s) instead of waiting out its 60-second rung: a pass puts it straight back, a failure changes nothing.

**It runs in the background and you can watch it.** A list of several hundred addresses takes minutes, so the press starts a job and returns; the page shows `Tested 212 of 834 · 41 working · 12 slow · 163 confirmed dead · 2 refused`, and `confirming 612 addresses (attempt 2 of 3)` while it re-tests, with a **Stop**. Stopping keeps everything that has already passed — it means "that is enough addresses", not "throw the work away". Reloading the page re-attaches to a sweep that is still running rather than losing it, and only one fetch runs at a time: a second press while one is going is refused, naming the one that is.

**Nineteen settings, all on Limits & Resilience → Credential health.** Six of them became settings in 7.24.0 and ship with exactly the number the code used before. The four added in 7.53.0 (`PROXY_CHECK_CONFIRM_ATTEMPTS`, `PROXY_CHECK_CONFIRM_SPACING_SECONDS`, `PROXY_CHECK_SLOW_MS`, `PROXY_CHECK_LINK_GUARD`) do change what a fetch does by default — see *Screen, then confirm* above. `PROXY_ORDER_RESORT_MINUTES` (7.56.0) paces the speed order, not a fetch.
...
| `PROXY_CHECK_TIMEOUT_SECONDS` | `10` | How long any single leg of an address check may take — the TLS handshake through the tunnel, and the `HEAD` that follows it when one is sent. Ten seconds is what every release up to 7.23.0 hard-coded. Raise it for a chain that has to reach the other side of the world, where a working but distant address is otherwise written off; lower it to give up on a slow address sooner. Range 1–120. This is the *check*, not the request path: a real request through a proxy is bounded by `PROXY_CONNECT_TIMEOUT_SECONDS`. Since 7.52.2 the background health re-prober honours it too; before, the re-prober always used ten seconds whatever this said. |
| `PROXY_CHECK_CONFIRM_ATTEMPTS` | `3` | How hard a check tries before it calls an address dead. A **fetch** re-tests everything its screen called dead — except a proxy that refused the connection outright — this many more times, with `PROXY_CONNECT_TIMEOUT_SECONDS` for the dial. The **health re-prober**, **Test all** and **Add all working** give an address this many tries *in total* and move it down the re-check ladder only when every try failed, once per round; there, `1` is every release before 7.53.0. So the number means "re-tests after the screen" for a fetch and "tries in all" for the others. The single-row **Test** is always one try. Measured on 1,000 feed addresses: a fetch called 775 dead, and three re-tests 30 s apart found 82, then 33, then 47 of them working — **+162 working, about 3.5 minutes added to the fetch**. Range 1–10. |
| `PROXY_CHECK_CONFIRM_SPACING_SECONDS` | `30` | How long MCC waits between one try of an address and the next — before the first confirm round, between rounds, and between the tries of a re-probe round. Thirty seconds is the spacing the measurement above used. `0` re-tests at once. Range 0–600. |
| `PROXY_CHECK_SLOW_MS` | `3000` | Setup time — connect, tunnel and TLS handshake added up — above which a passing address is labelled **slow** instead of **working**. A label, never a verdict: a slow address is kept and can be added to a chain. 3,000 ms marked the slowest 35 % of the passes a shipped fetch found on the measuring machine, and 58 % of the passes the re-tests recovered. Range 100–60000. |
| `PROXY_ORDER_RESORT_MINUTES` | `30` | With **Keep the fastest healthy proxy first** on for a chain, the shortest time between two orders MCC writes into it. **A stability choice, not a measurement**: together with the 25 % / 500 ms margin it keeps a chain from swapping back and forth between two addresses of similar speed, and it bounds how often that one provider is rebuilt for a new order. **Sort by speed now** is not held back by it. Range 5–1440. See *Keep the fastest healthy proxy first* below. |
| `PROXY_CHECK_LINK_GUARD` | `true` | Whether a fetch watches your own connection while it runs: one **direct** TLS handshake — no proxy, no request, ordinary strict trust — to the provider host it tests against, before the sweep and every 15 seconds during it. If that fails or takes longer than 5 seconds the fetch **pauses**, says *your own connection to … is failing — paused*, and marks nothing dead for checks that failed while the link was down (they are tried again once it answers). Not adaptive concurrency: at 8, 32 and 100 checks at once the same half hour found 181, 183 and 182 working addresses, so there was no congestion to back off from. |
| `PROXY_CHECK_MAX_CONCURRENCY` | `4` | How many addresses the **background health re-prober** tests at the same time — the loop that, since 7.19.0, is the only way a benched address gets back into the rotation. That loop is the single consumer; the fetch sweep and **Add all working** are paced by `PROXY_FETCH_TEST_CONCURRENCY`. Four is what 7.19.0–7.23.0 hard-coded. Each check sends the full end-to-end request to your provider's own host, which is why the ceiling is 128 rather than the fetch sweep's 500. Range 1–128. Before 7.52.2 the re-prober was clamped at four whatever this said, so only lowering it had any effect. |
...
A single verdict says little about a free proxy: an address that works at all passes about half its tries, and through the best proxy the first token of an answer arrived in 5.7 s while through the worst it took 186 s. So MCC now keeps, per address **and per provider**, a short history of how fast its connection sets up and how often it works, and the Proxying page can sort and filter by it. Nothing here reorders a chain; since 7.56.0 a chain can opt in to that — see *Keep the fastest healthy proxy first* below.
...
#### Keep the fastest healthy proxy first (7.56.0)

Picking a fast proxy matters more than anything else about a free one: through the best proxy the first token arrived in 5.7 s, through the worst in 186 s. Under `failover` a chain uses its **first** healthy entry, so the cheapest way to use the fast ones is to keep them at the top. That is what this switch does — and **all** it does: MCC rewrites the order of the chain's entries through the ordinary chain save. The selection engine itself is unchanged, byte for byte; failover still takes the lowest selectable entry, and every rule about benches, refusals and Direct applies exactly as before.

**The switch.** *Keep the fastest healthy proxy first*, on each chain card under its entries. It is **on for a chain you create from 7.56.0 on**, and **off for every chain you already had**: a chain saved before this release has no such setting, and MCC reads that as off, so your existing order is never rearranged until you click it. One click turns it on; that click writes only the switch and moves nothing. A save that does not mention the switch keeps what the chain had. It is offered for `failover` and `single` chains only — `round_robin` and `least_used` do not use the order to pick one address, so on those the switch is off, greyed out, and says why.

**What gets sorted.** Only entries that are **not paused, not Direct, not refused** (caught intercepting TLS), **not unhealthy** (benched by the reachability ladder) **and not holding a trigger bench** (the *cooldown* a 429, quota or timeout answer earns), by the rank described above for *this chain's provider*; lower is better, ties and addresses MCC has not measured keep the order you gave them. Everything else keeps its relative order below the sorted ones, so an unhealthy, refused or paused entry can only ever move **down**, never up. Two entries keep their exact place: **Direct** — counted from the end of the list, and the list never changes length — and an address on a **trigger bench**, which is neither promoted nor demoted while it is benched.

**When MCC writes a new order.** Once a minute it looks at every **enabled** chain with the switch on, and writes a new order only when **all** of these hold:

- the **first healthy entry** — the one failover actually uses — would change;
- the newcomer's rank is at most **0.75 ×** the current first entry's **and** at least **500 ms** better (a first entry MCC has no measured setup for loses to any measured newcomer);
- the newcomer has at least **3 samples** in the last 24 hours;
- no order was written into this chain in the last `PROXY_ORDER_RESORT_MINUTES` (default **30**).

A change further down the list is never written on its own: it rides along with a change of the first entry, or waits for **Sort by speed now**. So a re-sort is rare and does not flap between two addresses of similar speed. Each written re-sort is logged — `PROXY ORDER: <provider> re-sorted by speed (automatic): first <old> (rank N ms) -> <new> (rank M ms)` — and the card shows when it last happened. Writing an order is a chain save, so it rebuilds **that provider only** (see *Saving a chain rebuilds that provider* below); no other provider's pool, client or benches are touched, and the loop never makes a network request — it reads the measurements checks and live traffic already recorded.

**Two buttons, never automatic.**

- **Sort by speed now** sorts the saved chain once, by the same rules minus the three thresholds (margin, samples, interval). It still never moves an unhealthy, refused, paused or benched address up, and Direct keeps its place. It works whether the switch is on or off, and does not turn it on.
- **Pause all but the fastest N** pauses every address in the chain except the N best-ranked healthy ones. Nothing is removed and nothing is un-paused; each paused row's **Resume** (then **Save**) brings it back. It is refused, with a message, while MCC has measured no healthy address in the chain. MCC never does this by itself.

**Never touched by any of this:** the selection engine (`core/proxy_rotation.py`, `providers/runtime/proxy_rotating.py`), a chain with the switch off, a disabled chain (by the loop), what a pause, a refusal or a bench means, and which addresses are in a chain.
...
**Saving a chain rebuilds that provider, and only that one.** The address is read when a provider is constructed, so the save has to republish the provider generation for the chain to route — which resets *that provider's* credential pool, so its key health on the Providers page reads zeros immediately afterwards. The old numbers were not wrong; the pool they were measured on no longer exists. Since 7.55.0 every other provider keeps the very object it had across the save — its client, its credential pool with its counters and benches, its limiter and its proxy engine — because a chain is read by its own provider alone and saving one chain never rewrites another chain's addresses (an address two chains share is reused under one id). Removing a chain, the last batch of **Add all working**, and **Undo** after a bulk add are scoped the same way: an undo rebuilds exactly the providers whose chain it put back. A bulk add you **Stop** part-way rebuilds the providers its batches wrote; if a stopped run's rebuild is lost (the page was closed), the next chain save of any provider picks those up too. Saving settings, adding or editing a custom provider, and the reasoning-dialect probe still rebuild every provider, as they always did.
