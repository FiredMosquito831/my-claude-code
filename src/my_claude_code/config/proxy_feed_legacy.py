...
circulation can still hold a built-in id -- **7.28.0** is the earliest sensible
point. It used to say 7.21.0, then 7.22.0, then 7.23.0, then 7.24.0, then
7.25.0, then 7.26.0, then 7.27.0; all seven releases went to something else (the
...
server instead of replacing it, then the server no longer holding its own event
loop while it works), and deleting this in any of them would
