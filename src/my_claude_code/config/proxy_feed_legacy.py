...
circulation can still hold a built-in id -- **7.25.0** is the earliest sensible
point. It used to say 7.21.0, then 7.22.0, then 7.23.0, then 7.24.0; all four
releases went to something else (the proxy fetch pass, the 429 cooldown
controls, the Responses surface learning what a host refuses, then putting
every remaining proxy tunable on the dashboard), and deleting this in any of
them would have bundled an unrelated removal onto it. One concern per release,
