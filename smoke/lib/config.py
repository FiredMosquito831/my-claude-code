...
from my_claude_code.config.model_refs import parse_model_name, parse_provider_type
...
TARGET_REQUIRED_ENV: dict[str, tuple[str, ...]] = {
    "api": (),
    "auth": (),
    "cli": ("MCC_SMOKE_CLAUDE_BIN", "configured provider for Claude CLI prompt"),
    "clients": (),
    "config": (),
    "extensibility": (),
    "messaging": (),
    "providers": ("configured provider credentials/endpoints or MCC_SMOKE_MODEL_*",),
    ...
    "nvidia_nim_cli": (
        "NVIDIA_NIM_API_KEY",
        "MCC_SMOKE_CLAUDE_BIN or claude on PATH",
    ),
    "openrouter_free_cli": (
        "OPENROUTER_API_KEY",
        "MCC_SMOKE_CLAUDE_BIN or claude on PATH",
    ...
    "telegram": (
        "TELEGRAM_BOT_TOKEN",
        "ALLOWED_TELEGRAM_USER_ID or MCC_SMOKE_TELEGRAM_CHAT_ID",
    ),
    "discord": (
        "DISCORD_BOT_TOKEN",
        "ALLOWED_DISCORD_CHANNELS or MCC_SMOKE_DISCORD_CHANNEL_ID",
    ),
    "voice": ("VOICE_NOTE_ENABLED=true", "MCC_SMOKE_RUN_VOICE=1"),
}
...
class SmokeConfig:
    ...
    def load(cls) -> SmokeConfig:
        ...
        return cls(
            ...
            interactive=os.getenv("MCC_SMOKE_INTERACTIVE") == "1",
            targets=_parse_targets(os.getenv("MCC_SMOKE_TARGETS")),
            provider_matrix=_parse_csv(os.getenv("MCC_SMOKE_PROVIDER_MATRIX")),
            timeout_s=float(os.getenv("MCC_SMOKE_TIMEOUT_S", "45")),
            prompt=os.getenv("MCC_SMOKE_PROMPT", "Reply with exactly: MCC_SMOKE_PONG"),
            claude_bin=os.getenv("MCC_SMOKE_CLAUDE_BIN", "claude"),
    ...
    def mistral_reasoning_smoke_model(self) -> ProviderModel | None:
        ...
        if not self.has_provider_configuration("mistral"):
            return None
        override_env = "MCC_SMOKE_MODEL_MISTRAL_REASONING"
    ...
    def _include_provider_in_smoke(
        ...
        if self.provider_matrix and provider in self.provider_matrix:
            return True
        return bool(os.getenv(f"MCC_SMOKE_MODEL_{provider.upper()}"))
...
def _provider_smoke_model(provider: str) -> tuple[str, str]:
    override_env = f"MCC_SMOKE_MODEL_{provider.upper()}"
    if override := os.getenv(override_env):
...
def _normalize_provider_model(provider: str, raw_model: str) -> str:
    ...
    if not model:
        msg = f"MCC_SMOKE_MODEL_{provider.upper()} must not be empty"
    ...
    if prefix in SUPPORTED_PROVIDER_IDS and prefix not in _GATEWAY_HOSTED_NAMESPACES:
        msg = (
            f"MCC_SMOKE_MODEL_{provider.upper()} must use provider prefix "
...
def nvidia_nim_cli_model_refs(
    ...
    explicit_models = _parse_csv_ordered(source.get("MCC_SMOKE_NIM_MODELS"))
    extra_models = _parse_csv_ordered(source.get("MCC_SMOKE_NIM_EXTRA_MODELS"))

    if "MCC_SMOKE_NIM_MODELS" in source and not explicit_models:
        raise ValueError("MCC_SMOKE_NIM_MODELS must list at least one model")

    models: list[tuple[str, str]] = []
    base_models = explicit_models or NVIDIA_NIM_CLI_DEFAULT_MODELS
    base_source = (
        "MCC_SMOKE_NIM_MODELS" if explicit_models else "nvidia_nim_cli_default"
    )
    models.extend((model, base_source) for model in base_models)
    models.extend((model, "MCC_SMOKE_NIM_EXTRA_MODELS") for model in extra_models)
...
def openrouter_free_cli_model_refs(
    ...
    source = env if env is not None else os.environ
    explicit_models = _parse_csv_ordered(source.get("MCC_SMOKE_OPENROUTER_FREE_MODELS"))
    extra_models = _parse_csv_ordered(
        source.get("MCC_SMOKE_OPENROUTER_FREE_EXTRA_MODELS")
    )

    if "MCC_SMOKE_OPENROUTER_FREE_MODELS" in source and not explicit_models:
        raise ValueError(
            "MCC_SMOKE_OPENROUTER_FREE_MODELS must list at least one model"
        )
    ...
    models.extend((model, base_source) for model in base_models)
    models.extend(
        (model, "MCC_SMOKE_OPENROUTER_FREE_EXTRA_MODELS") for model in extra_models
    )
