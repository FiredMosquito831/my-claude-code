...
```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_TARGETS = "providers"
...
catalog/docs and can be overridden with `MCC_SMOKE_MODEL_<PROVIDER>`, for example
`MCC_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `MCC_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `MCC_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_TARGETS = "ollama"
...
```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_TARGETS = "telegram,discord,voice"
$env:MCC_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_TARGETS = "nvidia_nim_cli"
$env:MCC_SMOKE_NIM_MODELS = "z-ai/glm-5.2,moonshotai/kimi-k2.6,minimaxai/minimax-m2.7,nvidia/nemotron-3-super-120b-a12b,deepseek-ai/deepseek-v4-pro,deepseek-ai/deepseek-v4-flash"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_TARGETS = "openrouter_free_cli"
$env:MCC_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:FCC_LIVE_SMOKE = "1"
$env:MCC_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```
...
- `MCC_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `MCC_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `MCC_SMOKE_MODEL_NVIDIA_NIM`, `MCC_SMOKE_MODEL_OPEN_ROUTER`,
  `MCC_SMOKE_MODEL_MISTRAL`, `MCC_SMOKE_MODEL_MISTRAL_REASONING`,
  `MCC_SMOKE_MODEL_MISTRAL_CODESTRAL`,
  `MCC_SMOKE_MODEL_DEEPSEEK`, `MCC_SMOKE_MODEL_KIMI`,
  `MCC_SMOKE_MODEL_WAFER`, `MCC_SMOKE_MODEL_MINIMAX`,
  `MCC_SMOKE_MODEL_OPENCODE`, `MCC_SMOKE_MODEL_OPENCODE_GO`,
  `MCC_SMOKE_MODEL_ZAI`, `MCC_SMOKE_MODEL_FIREWORKS`, `MCC_SMOKE_MODEL_CLOUDFLARE`,
  `MCC_SMOKE_MODEL_GEMINI`, `MCC_SMOKE_MODEL_GROQ`, `MCC_SMOKE_MODEL_CEREBRAS`,
  `MCC_SMOKE_MODEL_OLLAMA_CLOUD`, `MCC_SMOKE_MODEL_LMSTUDIO`,
  `MCC_SMOKE_MODEL_LLAMACPP`, `MCC_SMOKE_MODEL_OLLAMA`: optional per-provider
  smoke model overrides. Values may include the provider prefix or just the model
  name for that provider.
- `MCC_SMOKE_MODEL_MISTRAL_REASONING`: optional override for the dedicated
  Mistral native reasoning smoke, default `mistral/mistral-medium-3-5`.
- `MCC_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `MCC_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `MCC_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `MCC_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `MCC_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `MCC_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `MCC_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `MCC_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `MCC_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `MCC_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
- `target_disabled`: skipped because `MCC_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `MCC_SMOKE_PROVIDER_MATRIX`;
