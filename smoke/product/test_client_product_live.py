...
def test_pi_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    with SmokeServerDriver(
        ...
        result = subprocess.run(
            [
                ...
                "--no-approve",
                "--print",
                "Reply with exactly MCC_SMOKE_PI",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
    ...
    assert "MCC_SMOKE_PI" in result.stdout
...
def test_claude_cli_adaptive_thinking_e2e(
    ...
    with SmokeServerDriver(
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="think hard, then reply with exactly MCC_SMOKE_CLI",
        )
        server_log = server.log_path.read_text(encoding="utf-8", errors="replace")
    ...
    assert "MCC_SMOKE_CLI" in result.stdout
...
def test_claude_cli_provider_error_e2e(
    ...
    with (
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="Reply with exactly MCC_SMOKE_UNREACHABLE.",
